...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        # use self.crossword.overlaps to find the overlap between x and y
        ...
        for x_word in self.domains[x].copy():
            ...
            for y_word in self.domains[y]:
                if x_word[i] == y_word[j]: #if the characters match nothing happens
                    match = True
                    break
            if not match:   #if no match was found, remove x_word from domain of x
    ...
    def ac3(self, arcs=None):
        ...
        # Process the queue
        while queue:
            x, y = queue.pop(0) # Dequeue an arc (x, y)
            if self.revise(x, y):
